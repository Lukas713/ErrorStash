...
export async function updateErrorEntry(input: {
  id: string
  userId: string
  title: string
  status: 'SOLVED' | 'UNSOLVED'
  ...
  solution?: string | null
  tags: string[]
}): Promise<ErrorEntryDetail> {
  const { id, userId, title, status, isPublic = false, description, stackTrace, solution, tags } = input

  // Upsert the user's tags so we have their ids.
  const tagRecords = tags.length > 0
    ? await Promise.all(
        tags.map(name =>
          prisma.tag.upsert({
  ...
  // Reconcile join rows against the entry's current tags.
  const current = await prisma.errorTag.findMany({
    where: { errorEntryId: id },
    select: { tagId: true },
  })
  const currentTagIds = new Set(current.map(c => c.tagId))
  const toRemove = [...currentTagIds].filter(tid => !newTagIds.has(tid))
  const toAdd = [...newTagIds].filter(tid => !currentTagIds.has(tid))

  const entry = await prisma.errorEntry.update({
    where: { id },
    data: {
      title,
      status,
      ...
      solution: solution ?? null,
      tags: {
        ...(toRemove.length > 0 && { deleteMany: { tagId: { in: toRemove } } }),
        ...(toAdd.length > 0 && { create: toAdd.map(tagId => ({ tagId })) }),
      },
    },
    include: {
      tags: { include: { tag: true } },
